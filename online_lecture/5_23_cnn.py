...
b1 = tf.Variable(tf.zeros((100, row, col, 16)), tf.float32)
...
b2 = tf.Variable(tf.zeros((100, 30, 24, 32)), tf.float32)
...
b3 = tf.Variable(tf.zeros((100, 15, 12, 64)), tf.float32)
conv3 = tf.nn.relu(tf.nn.conv2d(pool2, w3, [1, 1, 1, 1], "SAME") + b3)
pool3 = tf.nn.max_pool(conv3, [1, 2, 2, 1], [1, 2, 2, 1], "VALID")

flat = tf.reshape(pool3, (-1, 7*6*64))

w_fc1 = tf.Variable(tf.truncated_normal((2688, 1000)), tf.float32)
...
with tf.Session() as sess:
    ...
    sess.run(init)
    for i in range(100):
        _, _loss, _fc2, _label, _summ_str = sess.run([train, loss, fc2, label, summaries])
        writer.add_summary(_summ_str, i)
        if i % 10 == 0:
            print("step: ", i)
            print("loss: ", _loss)
            print("prediction: ", _fc2[:10])
